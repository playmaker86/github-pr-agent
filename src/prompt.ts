...
export function buildUserMessage(files: { filename: string; patch: string }[]): string {
  const diffs = files
    .map((f) => `### ${f.filename}\n\`\`\`diff\n${f.patch}\n\`\`\``)
    .join("\n\n");
  return `请审查以下文件变更：\n\n${diffs}`;
...
export function buildSummaryMessage(fileReview: { filename: string; review: string }[]): string {
